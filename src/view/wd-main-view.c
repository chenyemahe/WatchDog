...
static result_type _main_view_make_lock_layout(main_view *data, Evas_Object *parent);
...
Evas_Object *main_view_add(Evas_Object *navi)
{
    ...
    elm_object_part_content_set(view->layout, "elm.swallow.content_camera", view->preview_canvas);
...
result_type _main_view_make_lock_layout(main_view *data, Evas_Object *parent)
{
    ...
    elm_object_part_content_set(data->layout, "elm.swallow.content_lock", data->lock);
...
void _main_view_handle_key_press(main_view *view, key_type key)
{
	...
	if(_is_user_code_exist())
	{
		...
		{
			if(passc->pass_code_retry_count == 4)
			{
				_main_view_start_timer(view);
				_main_view_reset_user_code();
				passc->pass_code_retry_count = 0;
	...
	else
	{
		...
		if(key == CALC_KEY_0) {
			...
			substr(passc->user_passcode,passc->user_passcode,0,(strlen(passc->user_passcode)-1));
		}
		free(num);
	}
}
